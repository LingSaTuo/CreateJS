...
public class Console extends FrameLayout {
  ...
  final String REMOVING_UNSUPPORTED_MESSAGE
  ...
  /**
   * Write provided object String representation to console and starts new_data line
   ...
   */
  public void write(SpannableString spannableString) {
    buffer.append(spannableString);
    scheduleBufferPrint();
  }

  /**
   * Write Spannable to console and starts new_data line
   * "null" is written if the object is null
   *
   * @param spannableString SpannableString to write
   */
  public void writeLine(SpannableString spannableString) {
  ...
  void scheduleBufferPrint() {
    runBufferPrint();
  }

  private void runBufferPrint() {
    if (!isUIThread()) {
      getUIThreadHandler().post(new BufferPrintRunnable());
      return;
    }
  ...
  //region Nested classes
   ...
   class BufferPrintRunnable implements Runnable {
    @Override
    public void run() {
      runBufferPrint();
    }
