...
public class BTagNode extends BXMLNode {
	...
	public BTagNode getChildByName(StringBlock block, String name) {
		for (BXMLNode one : getChildren())
			if (name.equals(((BTagNode) one).getName(block))) return (BTagNode) one;
		return null;
	}

	public String getAttrStringForKey(StringBlock block, String name) {
		int node=block.putString(name);
		for (Attribute one : getAttribute())
			if (one.mName == node) return block.getStringFor(one.mString);
		return null;
	}

	public String getName(StringBlock block) {
		return block.getStringFor(getName());
	}
	...
	public void setAttribute(StringBlock block, String key, String value) {
		int qkey=block.putString(key);
		int qvalue=block.addString(value);
		setAttrStringForKey(qkey, qvalue);
		block.setString(qvalue, value);
	}
