...
public class ZipSigner 
{
    ...
    public void setKeymode(InputStream... inputStreams) throws IOException, GeneralSecurityException
    {
        if (getLogger().isDebugEnabled()) getLogger().debug("setKeymode: " + "");
        keymode = "platform";
        if (keymode.startsWith(MODE_AUTO)) {
            keySet = null;
        }
    ...
    public void loadKeys(InputStream... is) throws IOException, GeneralSecurityException {
        keySet = loadedKeys.get("platform");
        if (keySet != null) return;

        keySet = new KeySet();
        keySet.setName("platform");
        loadedKeys.put( "platform", keySet);

        if (KEY_NONE.equals("platform")) return;

        issueLoadingCertAndKeysProgressEvent();

        // load the private key
//        URL privateKeyUrl = getClass().getResource("/keys/"+name+".pk8");
        keySet.setPrivateKey(readPrivateKey(is[0], null));

        // load the certificate
//        URL publicKeyUrl = getClass().getResource("/keys/"+name+".x509.pem");
        keySet.setPublicKey(readPublicKey(is[1]));

        // load the signature block template
//        URL sigBlockTemplateUrl = getClass().getResource("/keys/"+name+".sbt");
        if (is[2] != null) {
            keySet.setSigBlockTemplate(readContentAsBytes(is[2]));
        }
    }
    ...
    public X509Certificate readPublicKey(InputStream inputStream)
            throws IOException, GeneralSecurityException {
    ...
    public PrivateKey readPrivateKey(InputStream inputStream, String keyPassword)
            throws IOException, GeneralSecurityException {
